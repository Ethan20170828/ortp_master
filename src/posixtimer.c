...
void posix_timer_do()
{
	...
	{
		tv.tv_sec = diff / 1000;	// 转换为多少秒
		tv.tv_usec = (diff % 1000) * 1000;	// 转换为多少微秒
#if	defined(_WIN32) || defined(_WIN32_WCE)
        /* this kind of select is not supported on windows */
		Sleep(tv.tv_usec / 1000 + tv.tv_sec * 1000);	// 休息多少ms
#else
...
// 将全局唯一的posix_timer定时器对象设为调度器的定时器。
// 这个posix_timer定时器是唯一可用的定时器，周期为50ms，
// 而且贯穿ortp源码也只有这里使用了这个定时器。
