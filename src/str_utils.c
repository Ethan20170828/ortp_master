...
*/
/* str_utils.c文件是一个队列，队列的实现相对复杂一点。
 * 首先，队列数据结构由三部分组成: 队列头、消息块以及数据块；
 * 队列头指向消息块，消息块之间可以构成双向链表，这是队列的基本要素。
 * 消息块本身不带buffer，数据是由专门的数据块来保存的，并被消息块指向。
************************************************************************/
...
// 从这个初始化一个queue_t的函数得知，_q_stopper初始化的时候，
// _q_stopper.b_next、_q_stopper.b_prev都是指向_q_stopper自己的。
