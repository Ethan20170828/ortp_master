...
static ortp_socket_t create_and_bind(const char *addr, int port, int *sock_family, bool_t reuse_addr){
	...
	for (res = res0; res; res = res->ai_next) {
		...
		if (reuse_addr){
			// SOL_SOCKET指的是通用套接字选项；
			// SO_REUSEADDR指的是允许重用本地地址和端口
	...
	*sock_family=AF_INET;
	...
	// 传参是SOCK_DGRAM表示我们传输层使用的是UDP协议；
	sock = socket (PF_INET, SOCK_DGRAM, 0);
	if (sock == -1) 
		return -1;

	// reuse_addr传参给的是false
	if (reuse_addr){
		err = setsockopt (sock, SOL_SOCKET, SO_REUSEADDR,
				(SOCKET_OPTION_VALUE)&optval, sizeof (optval));
		if (err < 0)
...
#endif
	// 把本地的IP地址和socket绑定起来，然后对网络文件描述符的属性进行修改
...
rtp_session_set_remote_addr_full (RtpSession * session, const char * rtp_addr, int rtp_port, const char * rtcp_addr, int rtcp_port)
{
...
#endif
	// session->rtp.socket = -1前面赋值为-1了，这里进入if循环
	...
	// 释放getaddrinfo接口返回的res指向的链表，如果调用getaddrinfo接口后没有释放内存，会造成
	// 内存泄露，所以一定要记得调用这个接口来释放内存；
	freeaddrinfo(res0);
	if (err) {
	...
	// 获取本地IP地址；
	// 通过res来返回一个指向struct addrinfo结构链表的指针(注意这里返回的是一个链表)；
...
// 上面的IP地址(IPv4,因为结构体使用的是sockaddr_in)和端口号设置完了，现在开始准备连接了；
	if (can_connect(session)){
		...
		// 连接，sa结构体中存放的是你要连接的服务的IP地址，这样我们就能找到服务器，并且能够连接上了；
		// 连接成功了，客户端和服务器就可以进行数据接收和发送了；
