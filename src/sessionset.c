...
**/
// 这函数是你用来新建SessionSet的，其中只是简单的分配空间，然后初始化。
...
// 这个函数是检测两个SessionSet之间有多少个相同的bit位都被置为1.
// 结果存放在第四个参数中，函数会将第一个参数和第三个参数中相同bit位之上都被置为1的数据
// 都复制到第四个参数中。也就是说如果第一个参数和第三个参数中某个bit位上都是1，那么第四个
// 参数中这个bit位也会被置1.第二个参数maxs表示传入的第一个参数和第三个参数中含有多少个需要比较的bit位。
int session_set_and(SessionSet *sched_set, int maxs, SessionSet *user_set, SessionSet *result_set)
{
	...
	while(i<maxs+1){
		...
		*mask1=(*mask1) & (~(*mask3));
		// 调用这个函数计算出结果元素中有多少位置1，并累加记录
		ret += count_power_items_fast(*mask3);
		// 因为i作为控制循环结束的关键变量代表的是结果集中剩余的bit位数，
		// 每个元素是4个字节也就是32个bit位。
		i+=32;
		mask1++;
		mask2++;
