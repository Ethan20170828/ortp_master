...
// 这个函数是用来检查等待点的时间是否到了，首先检查这个等待点的wakeup是否是真来决定是否需要被唤醒，
// 然后比较传入的时间是否等于等待点中存储的时间，如果是就代表需要唤醒并且把wakeup置为false防止重复唤醒。
...
// 执行rtp会话的一些必要的初始化工作
// 也就是填充struct _RtpSession结构体中的信息
void
...
**/
// 创建会话，其实就是一个结构体的实例化；用结构体来有很多元素，用这个结构体维护一个会话的所有信息
RtpSession *
 ...
 *
**/
// 设置会话的调度模式。当第二个参数为真时，表示会话可以使用调度模式，例如: 阻塞模式；
// RtpScheduler管理多个session的调度和收发的控制；
void
rtp_session_set_scheduling_mode (RtpSession * session, int yesno)
{
	if (yesno)
	{
		...
		if (sched != NULL)
		{
			// 给这个会话设置标志，表示该会话已经获取了调度器
			rtp_session_set_flag (session, RTP_SESSION_SCHEDULED);
			session->sched = sched;
			// 把当前的会话放到调度器中，被调度器管理，方便以后的执行
 ...
 *
 * @param session 	a rtp session RTP会话结构体
 * @param signal_name	the name of a signal 信号的名称
 * @param cb		a RtpCallback 回调函数
 * @param user_data	a pointer to any data to be passed when invoking the callback.传递给回调函数的数据
 *
**/
// 本函数提供一种方式，用于通知应用程序各种可能发生的RTP事件(信号)。
// 可能通过注册回调函数的形式来实现本功能。
int
 ...
 *	If the actual payload type in incoming packets is different that this expected payload type, thus
 *	the "payload_type_changed" signal is emitted(释放).
 *
 * @param session a rtp session
 * @param paytype the payload type number
 * @return 0 on success, -1 if the payload is not defined.
...
int rtp_session_set_payload_type(RtpSession *session, int pt){
	if (rtp_session_set_send_payload_type(session,pt) < 0) 
		return -1;
	if (rtp_session_set_recv_payload_type(session,pt) < 0) 
		return -1;
	return 0;
}
...
__rtp_session_sendm_with_ts (RtpSession * session, mblk_t *mp, uint32_t packet_ts, uint32_t send_ts)
{
	...
	// 保存了从开始发送到发送这个rtcp报告包时发送的字节总数，但不包括头部和填充
...
#else
	// buffer中存放的就是纯网络数据，但是真正的网络中发送数据是需要包头、包尾、校验和等信息的；
	// m就是纯网络数据添加完包头、包尾、校验和等信息后的数据。
...
void rtp_session_set_time_jump_limit(RtpSession *session, int milisecs){
	...
	if (ts==0) 
		session->rtp.ts_jump=1<<31;	/* do not detect ts jump */
	else 
		session->rtp.ts_jump=ts;
}
 ...
 *	if the session does symmetric(对称的) rtp (see rtp_session_set_symmetric_rtp())
 *	a the connect() is done to the source address of the first packet received.
 *	Connecting a socket has effect of rejecting(拒绝) all incoming packets that 
...
// 首先调用wait_point_check函数检查发送等待点的时间是否到了，这里用来比较wp里的时间
// 是在发送函数中计算好的，time是从参数传入的，从rtp_scheduler_schedule函数的介绍中可以
// 看出这个时间是当前时间。如果时间到了那么就在调度器的w_sessions发送集合中将代表这个
// rtpsession的bit位置1，然后唤醒可能在等待的wp里的条件变量。
