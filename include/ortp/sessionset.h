...
struct _SessionSet
{
	// rtpset里面其实就是long型的数组，长度是1024/(8*sizeof(long))，
	// 因为ortp用一个bit位标识一个rtpsession，第几个bit位就代表第几个rtpsession。
	// rtpset里面的数组计算出的长度刚好是1024bit位也就是32字节，也就是说一个SessionSet
	// 最多只能管理1024个rtpsession
	ortp_fd_set rtpset;
};
