...
typedef struct msgb
{	
	// b_prev和b_next: 消息块的链表指针，所有消息块通过这两个指针串联接成链。
	struct msgb *b_prev;	// 指向上一个消息块
	struct msgb *b_next;	// 指向下一个消息块
	// 通过这个指针可以构成复合消息块，复合消息块由多个消息块组成，但在队列中作为一个元素
	...
	uint32_t reserved1;		// 保留用途
	uint32_t reserved2;		// 保留用途
#if defined(ORTP_TIMESTAMP)
...
{
	// 消息块，内嵌于queue_t，它的主要作用是作为链表的表头；
	// _q_stopper所指向的mblk_t并无实际数据，只是作为消息队列的标识头。
	// putq或者getq都是从_q_stopper之后的第一个mblk_t开始。
